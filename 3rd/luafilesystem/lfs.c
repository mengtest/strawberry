...
static int combine_dir(lua_State *L) {
	size_t fl1, fl2;
	const char *f1 = luaL_checklstring(L, 1, &fl1);
	const char *f2 = luaL_checklstring(L, 2, &fl2);
	char *path = malloc(fl1 + fl2 + 2);
	memset(path, 0, fl1 + fl2 + 2);
	memcpy(path, f1, fl1);
#if defined(_WIN32)
	path[fl1] = '\\';
#else
