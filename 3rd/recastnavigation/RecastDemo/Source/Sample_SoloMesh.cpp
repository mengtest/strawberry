...
bool Sample_SoloMesh::handleBuild()
{
	...
	if (m_cfg.maxVertsPerPoly <= DT_VERTS_PER_POLYGON)
	{
		...
		if (!dtCreateNavMeshData(&params, &navData, &navDataSize))
		{
			...
			char* buf = 0;
			FILE* fp = fopen("exporter/hello.txt", "wb");
			if (fp != NULL) {
				fwrite(navData, 1, navDataSize, fp);
				fclose(fp);
			}
		}
		...
		int buffersz = 0;
		FILE* fp = fopen("exporter/hello.txt", "rb");
		if (fp != NULL) {
			buffersz = fread(buffer, 1, 1024 * 20, fp);
			SDL_assert(buffersz <= 1024 * 20);
			fclose(fp);
		}

		status = m_navMesh->init(buffer, buffersz, DT_TILE_FREE_DATA);
